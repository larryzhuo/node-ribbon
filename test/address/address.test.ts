...
describe('address list', () => {
  ...
  test('test random req', async () => {
    ...
    //set loadbalancer
    let lbIns = LoadBalancerFactory.getLoadBalancer({ type: LoadBalancerEnum.Random });
    ribbonIns.withLoadBalancer(lbIns);

    let httpClient = await ribbonIns.getHttpClient({
      fallback: (err) => {
    ...
    try {
      for (let i = 0, ilen = 10; i < ilen; i++) {
        let ret = await httpClient.get('/hello');
  ...
  test('test roundrobin req', async () => {
    ...
    //set loadbalancer
    let lbIns = LoadBalancerFactory.getLoadBalancer({ type: LoadBalancerEnum.RoundRobin });
    ribbonIns.withLoadBalancer(lbIns);
